...
export default {
    ...
    withdraw() {
      ...
      self.$vux.confirm.show({
        ...
        onConfirm() {
          ...
          api.user.DangNeiGuanAi.delete({
            ID: self.id
          })
            .then(function(res) {
              console.log('api.user.DangNeiGuanAi.delete:', res);
    ...
    update() {
      ...
      api.user.DangNeiGuanAi.update({
        ...
        .then(function(res) {
          ...
          if (res.StatusCode === 1200) {
            let alertContent = '更新成功，请等待审核，审核通过前你可以重新编辑';
            self.$vux.alert.show({
              title: '等待审核',
  ...
  mounted() {
    ...
    self.$nextTick(() => {
      ...
      api.user.DangNeiGuanAi.detail({ ID: self.id }).then(res => {
